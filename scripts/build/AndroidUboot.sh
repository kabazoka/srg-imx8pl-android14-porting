...
build_imx_uboot()
{
	...
	if [ "`echo $2 | cut -d '-' -f2`" = "trusty" ]; then
		...
		make -C ${IMX_PATH}/arm-trusted-firmware/ CROSS_COMPILE="${ATF_CROSS_COMPILE}" PLAT=`echo $2 | cut -d '-' -f1` bl31 -B SPD=none LPA=${POWERSAVE_STATE} IMX_ANDROID_BUILD=true IMX_BOOT_UART_BASE=0x30A60000 1>/dev/null || exit 1
	fi
